# Pre-Launch Checklist — Country Flags

> Copied from `docs/templates/07-CHECKLIST.md`.
> Work through this before every release. A phase is not done until every item is checked.
...
- [ ] Code follows project conventions from `docs/templates/03-ARCHITECTURE.md`
...
- [ ] Navigation state is preserved when switching tabs (if applicable)
...
- [ ] APK/AAB is reasonable size (< 50MB)

## Store Readiness

- [ ] Store description is accurate and follows template
- [ ] At least 4 phone screenshots taken on clean emulator (no debug banner)
- [ ] 7-inch tablet screenshots taken
- [ ] 10-inch tablet screenshots taken
...
- [ ] App categorized correctly in store (Education)

## CI/CD Pipeline

- [ ] `codemagic.yaml` updated with `country-flags-android` workflow
...
- [ ] App icon copied to `website/apps/country_flags/icon.png`
- [ ] 5 phone screenshots copied to `website/apps/country_flags/screenshots/`
- [ ] App entry added to `website/apps-data.js`
- [ ] `androidUrl` is correct live store link
- [ ] `iosUrl` is null (iOS not yet live, set when live)
- [ ] Website verified locally and in Chrome
- [ ] Commit pushed — GitHub Actions deploy workflow completed successfully
- [ ] Live site at `https://coziahr.com/foreverfree` confirmed working
